...
pub fn parse_elf(path: &str) -> Result<ParsedImage, Box<dyn Error>> {
    ...
    if !matches!(
        ...
        return Err(format!(
            "unsupported_arch: ELF architecture {:?} (only x86/x64 supported)",
            obj.architecture()
        )
        .into());
    }
...
fn collect_imports(obj: &object::read::File<'_>) -> Vec<ImportRecord> {
    ...
    for import in imports {
        ...
        let dll = String::from_utf8_lossy(lib_bytes).to_string();
